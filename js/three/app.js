...
const HAND_RADIUS_X = 5.0;
const HAND_RADIUS_Z = 3.0;
...
function createTable() {
  const tableGeo = new THREE.BoxGeometry(TABLE_W, 0.28, TABLE_H);
  ...
  const feltGeo = new THREE.BoxGeometry(TABLE_W - 0.5, 0.035, TABLE_H - 0.5);
  ...
  const groundCollider = RAPIER.ColliderDesc.cuboid(TABLE_W / 2, 0.05, TABLE_H / 2);
...
function createBoundaries() {
  ...
  const walls = [
    { x: 0, z: TABLE_H / 2, sx: TABLE_W / 2, sz: wallThickness },
    { x: 0, z: -TABLE_H / 2, sx: TABLE_W / 2, sz: wallThickness },
    { x: TABLE_W / 2, z: 0, sx: wallThickness, sz: TABLE_H / 2 },
    { x: -TABLE_W / 2, z: 0, sx: wallThickness, sz: TABLE_H / 2 }
  ];

  walls.forEach((wall) => {
    const body = app.world.createRigidBody(
      RAPIER.RigidBodyDesc.fixed().setTranslation(wall.x, wallY, wall.z)
    );
    const collider = RAPIER.ColliderDesc.cuboid(wall.sx, wallHeight / 2, wall.sz);
...
function createDropZones() {
  ...
  const tableZone = makeZone('table', 0, 0, TABLE_W - 0.8, TABLE_H - 0.8, 0x1d5d8f, 0.12);
...
function drawCardToPlayer(playerId, animateDraw = true) {
  ...
  tossTo(card, target, getHandRotation(playerId), animateDraw ? 0.34 : 0);
  updateHud();
}
...
function onPointerMove(event) {
  ...
  next.x = THREE.MathUtils.clamp(next.x, -TABLE_W / 2 + 0.4, TABLE_W / 2 - 0.4);
  next.z = THREE.MathUtils.clamp(next.z, -TABLE_H / 2 + 0.4, TABLE_H / 2 - 0.4);
  next.y = 0.56;
...
function moveCardToPlayer(card, playerId) {
  ...
  const target = getHandCardPosition(playerId, state.players[playerId - 1].cards.length - 1);
  tossTo(card, target, getHandRotation(playerId), 0.22);
  updateHud();
}
...
function moveCardToTable(card) {
  ...
  card.body.setTranslation({ x: pos.x, y: 0.65, z: pos.z }, true);
...
function getHandCardPosition(playerId, cardIndex) {
  const seat = getPlayerSeatPosition(playerId);
  const player = state.players[playerId - 1];
  const count = Math.max(player.cards.length, 1);
  const spread = Math.min(1.2, (count - 1) * 0.22);
  const offset = (cardIndex - (count - 1) / 2) * Math.min(0.38, spread || 0.3);
  const angle = getPlayerAngle(playerId);
  const side = new THREE.Vector3(Math.cos(angle), 0, -Math.sin(angle));

  return new THREE.Vector3(
    seat.x + side.x * offset,
    0.14 + cardIndex * 0.012,
    seat.z + side.z * offset
  );
}

function getHandRotation(playerId) {
  return -getPlayerAngle(playerId) + Math.PI / 2;
}

function getPlayerSeatPosition(playerId) {
  const angle = getPlayerAngle(playerId);
  return {
    x: Math.cos(angle) * HAND_RADIUS_X,
    z: Math.sin(angle) * HAND_RADIUS_Z
  };
}
...
function updateHud() {
  ...
  if (app.deckMesh) {
    app.deckMesh.scale.y = Math.max(0.12, state.deck.length / 30);
